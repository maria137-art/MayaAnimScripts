...
def clear_keys():
    ...
    If no attributes are highlighted, all attributes will be cleared.
    """
        ...
        for item in sel:
            for attr in selected_attributes:
                if cmds.attributeQuery(attr, node=item, exists=True):
                    cmds.cutKey(f"{item}.{attr}")
                else:
                    continue
    else:
        # If no attributes are selected, clear all keys for selected objects.
        cmds.cutKey(sel)
